...
public class BabyBombExplosion : SkillBehaviour
{
    ...
    public override IEnumerator Activate(GameObject attacker, GameObject target)
    {
        ...
        // TÌM CHÍNH XÁC PHẦN "BODY" ĐỂ CO GIÃN MÀ KHÔNG LÀM MÉO UI
        Transform body = attacker.transform.Find("Body");
        if (body == null) body = attacker.transform; // Dự phòng

        SpriteRenderer sr = body.GetComponent<SpriteRenderer>();

        // Nếu target bị mất kết nối, tìm lại
        if (target == null)
        {
        ...
        // Nhún người thu lại lấy đà (CHỈ SCALE BODY)
        body.DOScale(new Vector3(0.7f, 1.5f, 1f), 0.15f);
        yield return new WaitForSeconds(0.15f);

        // Ép dẹt người và phóng vút đi. Lưu ý: Di chuyển (DOMove) thì vẫn di chuyển gốc, còn Scale thì Scale Body
        ...
        // Phình to từ từ và rung bần bật
        ...
        if (bombLogic != null) bombLogic.OnExploded();
